...
var formatV2MarkerPath = "/var/run/aws-node/.npa_format_v2"
...
func renamePinFamily(progsDir, mapsDir, legacyID, newID string) (renamed int, failed int) {
	for _, suffix := range progPinSuffixes() {
		switch renamePinIfExists(filepath.Join(progsDir, legacyID+"_"+suffix), filepath.Join(progsDir, newID+"_"+suffix)) {
		case renameOK:
			renamed++
		case renameFailed:
			failed++
	...
	for _, suffix := range mapPinSuffixes() {
		switch renamePinIfExists(filepath.Join(mapsDir, legacyID+"_"+suffix), filepath.Join(mapsDir, newID+"_"+suffix)) {
		case renameOK:
			renamed++
		case renameFailed:
			failed++
